...
class MessageListener(val main: VoltrixSupportBot, val jda: JDA, val embeds: PremadeEmbeds) : ListenerAdapter() {
    ...
    override fun onMessageReceived(e: MessageReceivedEvent) {
        ...
        if (e.channel.idLong == main.config.getJSONObject("auto-support").getLong("channel-id")) {
            ...
            if (Utils.containsWordsIfQuestion(message, arrayOf("download", "install")) or Utils.containsPhrases(message, arrayOf("download client", "client link", "download link"))) {
                // Download client
            ...
            if(Utils.containsWordsIfQuestion(message, arrayOf("wiki", "docs", "install", "installing", "installed", "error"))
                or Utils.containsPhrases(message, arrayOf("dont know how", "help install", "how install", "help error", "dont work", "what do install"))) {
            ...
            if(Utils.containsWordsIfQuestion(message, arrayOf("support", "ticket"))
                or Utils.containsPhrases(message, arrayOf("staff support", "voltrix support", "client support", "report error"))) {
            ...
            if(Utils.containsPhrases(message, arrayOf("create mod", "add own mod", "install mod", "implement mod"))) {
